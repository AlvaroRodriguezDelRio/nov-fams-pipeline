...
The starting point of this pipeline consist a the concatenated fasta file with the gene predictions for the genomes of interest (both for proteins a CDS), which can be genrated with ```multifasta.format.py```. The scripts presented here assume that the gene names are formatted as ```>genome_source_of_isolation@genome_name@gene_name@domain|phylum```. This can be done with the ```multifasta.format.py``` script as follows:

```python multifasta.format.py -t genome2taxonomy.tab -p paths_genes.txt -ext .fasta -r -2 > multifasta.faa```
```python multifasta.format.py -t genome2taxonomy.tab -p paths_cds.txt -ext .fasta -r -2 > multifasta.fna```
...
-```-p``` reads a the paths to the gene predictions from each of the genomes (one file per genome)
-```-ext``` indicates the extention of the gene prediction files.
...
Run MMseqs2 for calculating the gene families on the concatenated proteomes of the genomes of interest (we used the  ```--min-seq-id 0.3 -c 0.5 --cov-mode 1 --cluster-mode 2 -e 0.001``` parameter combination). MMseqs provides a tsv file with two columns, ```gene_family_representative```, and ```gene_family_member``` (one row per gene in the dataset). This can be turned into a single line per gene family, including all gene family members, with ```datamash -g1 countunique 2 collapse 2```, which will provide a 3 column file: ```gene family representative```, ```number of members``` and ```gene family members (comma-separated)```
...
We considered as gene families exclusive from uncultivated taxa those with no member with significant hits to any of these databases. We named the file including all the novel gene familes ```clusters.folded.novel.tab```

## Quality filtering of novel gene families from uncultivated taxa

For dealineating the unknown gene family predictions of higher quality, we conducted a series of filtering steps, for which we needed to create individual fasta files with the i) CDS sequences and ii) Protein sequences of each gene family. For doing so, the ```fasPerFam.py``` script may be used. This script will create individual files for each novel gene family in the indicated directory, creating subdirectories for avoiding large number of files within a single folder:

```python fasPerFam.py -e .faa -f clusters.folded.novel.tab -p multifasta.faa -n faa```
...
- Get an ordered list of the genes, using the ```neighs_per_contig.py``` script

```python neighs_per_contig.json.py -p paths_genes_gff.txt -e .gff -r -2 > genes_per_contig.json```

- ```-p``` indicates the path to a file which contains the paths to the gff predictions of all genomes, and should look like:
```
...
- Upload the ```neighs_per_contig.json``` data to a Mongo (https://www.mongodb.com/) collection with:
 ...
 ```cat neighs_per_contig.json | mongoimport --host HOST_NAME -d DATABASE_NAME -c neighs --drop```.
